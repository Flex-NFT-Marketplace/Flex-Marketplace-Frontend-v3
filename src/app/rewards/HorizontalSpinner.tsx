...
import { isDigit } from "@/utils/string";
...
const HorizontalSpinner = () => {
  ...
  const spinWheel = (targetGift: number | undefined | string) => {
    ...
    if (typeof targetGift === "string" && !isDigit(targetGift)) {
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-10 border-t border-line bg-dark-black px-6 py-4 ">
        <p className="font-bold uppercase text-buy">
          Completed 5 tasks to free-spin |||||
        </p>
        <div className="flex gap-1">
          <Button
            onClick={() => spinWheel(target)}
            className="w-[120px] border-primary text-primary"
            variant="outline"
            title="Free-spin"
          />
          <Button
